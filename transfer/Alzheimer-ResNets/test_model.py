...
def test(file, class_names, data_dir, results_dir):
    ...
    train_size = int(0.0 * len(adni_dataset))
    ...
    for idx in range(len(predicted)):
        if predicted[idx] in [1, 2, 3]:
            predicted[idx] = 1
        if predicted[idx] in [4]:
            predicted[idx] = 2

        if predicted[idx] == 1:
            mcis[test[idx]] += 1
        else:
            fintest.append(min(1, test[idx]))
            finpred.append(min(1, predicted[idx]))

    print(mcis)

    # Imprime estadísticas y gráficos
    print_info_and_plots(fintest, finpred, ['CN', 'AD'])
...
if __name__ == '__main__':
    ...
    test(file='cuda-alzheimer-densenet121.pth', class_names=['CN','EMCI','MCI','LMCI','AD'],
         data_dir='./NumpyOrigOASIS', results_dir='./results')