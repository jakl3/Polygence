...
def test(class_names, data_dir, results_dir, epochs, batch_size, lr_decay_epochs=None, model_file=None, architecture='densenet121', plot_accuracy=None, nesterov=False):
    ...
    train_dataset, test_dataset = torch.utils.data.random_split(adni_dataset, [train_size, test_size],
                                                                generator=torch.Generator().manual_seed(41025607))
    ...
    test_train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=1, num_workers=12)
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=1, num_workers=12)
    ...
    torch.save(model.state_dict(),
               results_dir + '/' + device.type + '-epoch-' + 'test' + '-alzheimer-' + architecture + '.pth')
    ...
    while epoch < epochs:
        print("-"*60)
        print(f"Currently on epoch {epoch+1}/{epochs}")
        print(f"{time.time() - start_time:.2f} seconds have passed")
        if lr_decay_epochs is not None:
            lr_scheduler(optimizer, epoch, lr_decay=0.1, lr_decay_epochs=lr_decay_epochs)
        running_loss = 0.0
        for data in tqdm(train_loader, desc=f"training model on epoch {epoch+1}/{epochs}", file=sys.stdout):
        ...
        print('[epoch %d] loss: %.6f' % (epoch+1, running_loss / train_size))
        ...
        with open(results_dir+'/'+device.type+'-epoch-'+str(epoch)+'-losses.dump', 'wb') as losses_file:
            ...
            losses_file.close()
        if plot_accuracy and epoch % 5 == 0:
            print(f'now evaluating model at epoch {epoch+1}')
            model.eval()
            ...
            print('[epoch %d] accuracy: %.6f (test) %.6f (train)' % (epoch+1, accuracy_test, accuracy_train))
